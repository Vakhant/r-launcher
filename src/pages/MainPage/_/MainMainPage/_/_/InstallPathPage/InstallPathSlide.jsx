...
const InstallPathSlide = ({strt, name, space, chosen, id}) => {
    return (
        <div className={css.ipth_slider_block + ` ${chosen?css.ipth_slider_block_chosen:''} ${strt?css.ipth_slider_block_strt:''}`}>
            ...
            <svg className={css.ipth_slider_disk_ico} width="42" height="46" viewBox="0 0 42 46" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            {chosen?'Выбран':'Выбрать'} <div className={css.ipth_slider_disk_choose_box}>
                    {chosen?<><img src={process.env.PUBLIC_URL+'/imgs/icons/main_page/disk_mini-check-mark.svg'} alt="" /><img src={process.env.PUBLIC_URL+'/imgs/icons/main_page/disk_mini-check-mark.svg'} alt="" /></>:''}
                </div>
            </div>
        </div>
    )
